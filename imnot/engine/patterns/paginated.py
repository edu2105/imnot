...
def make_paginated_handler(
    ...
    style = (datapoint.pagination or {}).get("style", "offset_limit")
    if style == "cursor":
        return _make_cursor_handler(partner, datapoint, endpoint, store, default_limit)
    elif style == "page_number":
        return _make_page_number_handler(partner, datapoint, endpoint, store, default_limit)
    else:
        return _make_offset_limit_handler(partner, datapoint, endpoint, store, default_limit)


def _make_offset_limit_handler(
    partner: str,
    datapoint: DatapointDef,
    endpoint: EndpointDef,
    store: SessionStore,
...
def _make_cursor_handler(
    ...
    handler.__name__ = f"paginated_cursor_{partner}_{dp_name}"
    return handler


def _make_page_number_handler(
    partner: str,
    datapoint: DatapointDef,
    endpoint: EndpointDef,
    store: SessionStore,
    ...
    handler.__name__ = f"paginated_page_{partner}_{dp_name}"
    return handler
